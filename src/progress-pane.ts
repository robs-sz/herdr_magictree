...
import { capLine, elapsedText, logQuiet } from "./log.ts";
...
/** Enough to reach the last line of a run log without reading the transcript. */
 ...
 * `magictree up` can write megabytes of build output, and this runs every
 * second for as long as the run does.
 */
function lastLogLine(path: string): string {
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (line.length > 0) return capLine(line, TAIL_CHARS);
    }
  } catch {
    // No log yet, or it is mid-write: the pane renders without a tail line.
  } finally {
...
function watch(key: string): Watch {
  ...
  return { record, tail: record?.log_path ? lastLogLine(record.log_path) : "" };
}
